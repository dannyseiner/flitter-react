...
api.get('/getfriends/:id', (req, res) => {
    ...
    INNER JOIN accounts ON account_friends.id_user1 = accounts.account_id 
    WHERE account_friends.id_user1 = ${req.params.id} UNION SELECT * FROM account_friends 
    INNER JOIN accounts ON account_friends.id_user2 = accounts.account_id WHERE account_friends.id_user2 = ${req.params.id}`, (err, result) => {
        res.send(result)
    })
})
